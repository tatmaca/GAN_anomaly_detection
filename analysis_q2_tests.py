...
OUT_DIR = "q2_stats"
CODE_RE = re.compile(r"(?P<code>\d{4})_")
...
def infer_code_from_path(path: str) -> str:
    ...
    if not m:
        raise ValueError(f"Could not infer 4-digit code from filename: {path}")
    return m.group("code")

PAIRS = [
    # ("path/to/ABNORMAL.csv", "path/to/NORMAL.csv"),
    ("sorted_returns_for_matt/0050_abnormal_minutes_20251206_222509.csv",
     "sorted_returns_for_matt/0050_normal_minutes_20251206_222509.csv"),

    ("sorted_returns_for_matt/0056_abnormal_minutes_20251206_224120.csv",
     "sorted_returns_for_matt/0056_normal_minutes_20251206_224120.csv"),

    ("sorted_returns_for_matt/2330_abnormal_minutes_20251206_222550.csv",
     "sorted_returns_for_matt/2330_normal_minutes_20251206_222550.csv"),
]
...
def main():
    ...
    combined = []
    for ab_path, nm_path in PAIRS:
        code_ab = infer_code_from_path(ab_path)
        code_nm = infer_code_from_path(nm_path)
        if code_ab != code_nm:
            raise ValueError(f"Code mismatch: {ab_path} vs {nm_path}")

        code = code_ab
        print(f"\n[{code}] Reading:")
        print("  abnormal:", ab_path)
        print("  normal  :", nm_path)

        abnormal = pd.read_csv(ab_path)
        normal = pd.read_csv(nm_path)

        results = run_all_tests(abnormal, normal)
